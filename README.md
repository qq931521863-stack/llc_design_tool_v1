...
| 2026-09-15 / V9.2.2 | 🚀 | **多语言界面**：简体中文 / English / 日本語 / 한국어。工具栏最右侧「语言 / Language」，切换立即生效并记忆；界面原有的英文（Bode、H(z)、C99、Kp、PM、FRA…）按约定保持原样不翻译；帮助标题与正文按章节翻译，未译章节显示原文并明确提示；字体栈随语言切换（日文 Meiryo / Yu Gothic、韩文 Malgun Gothic、中文雅黑 / 思源），避免共用汉字显示成中文字形 |
| 2026-09-15 / V9.2.2 | 🚀 | 帮助 **48 个章节全部四语言完成**（含实现说明、模型边界、假设与限制的完整正文），运行期对话框/状态栏/日志文案约 60 条一并纳入 |
| 2026-09-15 / V9.2.2 | 🧪 | 新增 i18n 回归：把四个工作区实际绑定的**全部**界面字符串抓出来，要求每种语言都有译文（新增工具栏/页签若忘记配译文会直接测试失败）；帮助章节回退提示、语言菜单勾选、字体栈、语言记忆均有测试 |
...
界面支持 **简体中文 / English / 日本語 / 한국어**，切换位置在任意工作区工具栏最右侧的
「语言 / Language」，选择后**立即生效**并写入设置，下次启动沿用。

* **界面中原本就是英文的部分保持原样**（`Bode`、`H(z)`、`C99`、`Kp`、`PM`、`FRA`、`Summary` 等）。
  工程术语的一致性比逐字翻译更重要，因此这些词在任何语言下都不翻译。
* **帮助全部章节四语言完整**：48 个章节（含全部「实现说明」与「模型边界」正文）在
  简体中文 / English / 日本語 / 한국어 下都已翻译，无缺章。
* **运行期文案也已纳入**：对话框标题与正文、状态栏与运行日志、导出提示等约 60 条
  已接入同一条翻译链路。
* 若某个字符串将来确实没有译文，会原样显示原文，帮助正文则额外在正文上方提示，
  不会静默给出中英混杂却不说明的内容。
* **字体栈随语言切换**：日文 Meiryo / Yu Gothic、韩文 Malgun Gothic、中文微软雅黑 / 思源，
  避免共用汉字（如 直 / 骨 / 次）被渲染成中文字形；matplotlib 图表字体同步切换。
...
* **实现说明** — 算法到底是什么算的：FHA/HB/TD 的求解方式、数字环的混合域求值与延时拆分、
  Tustin/预畸变/后向欧拉的映射公式、C99 DF2T/SOS 结构与系数符号、稳定性判定阈值、
  FRA 的剥离与重建、变量投影有理拟合、ZOH 闭环与 z⁻¹/z 系数对齐、Auto Design 综合流程；
* **模型边界与已知限制** — 每个工作区声明的假设与不能用来声称的结论；
* **快捷键与操作**；
* **联系方式与支持** — 邮箱 / 微信 / 公众号二维码，以及发邮件时该附什么信息；
* **打开文档** — 本地 Markdown 存在时直接打开，打包版本自动回退到 GitHub 上的同名文档。

模型边界与实现说明是帮助的重点：只有知道一个数字是怎么算出来的、它隐含了哪些假设，
才能判断它能不能用来支持一个设计结论。
...
| | |
| --- | --- |
| **邮箱（推荐）** | **maileyang@qq.com** |
| **微信** | **maileyang** |
| **公众号 / 技术博客** | **开关电源仿真与实用设计** |

<img src="llc_design/data/wechat_official_account.jpg" alt="微信公众号：开关电源仿真与实用设计" width="220">

**有不懂的地方、拿不准的取舍，或者发现结果与实测/理论不符，欢迎直接发邮件给我。**
工程问题往往要结合具体拓扑、工况和波形才能判断，讨论和研究通常比读文档更快，
我也很希望知道这些工具在真实项目里到底卡在哪里。
...
4. 若与实测相关（尤其 FRA），请说明测量方式、注入点，以及数据是否包含当前控制器。

要提醒的是：不少「看起来不对」其实属于**模型边界**而不是缺陷，例如 FHA 远离谐振点、
采样 `Ms`/`Mt` 落在扫频点之间、Complete Loop 没填真实 `C_old`。
这些在对应工作区的「模型边界与已知限制」（帮助里，或下文各节）都有说明。

程序内路径：任意工作区工具栏 **帮助（F1）→ 联系方式与支持**。
